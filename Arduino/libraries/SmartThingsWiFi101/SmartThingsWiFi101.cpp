...
namespace st
{
	...
	void SmartThingsWiFi101::init(void)
	{
		...
		st::debugPrintln(F(""));
		st::debugPrintln(F("Initializing WiFi 101 network.  Please be patient..."));

		// check for the presence of the shield
		if (WiFi.status() == WL_NO_SHIELD) {
			st::debugPrintln("WiFi 101 shield not present");
			// don't continue
			while (true);
		}
		...
		st::debugPrintln(F(""));
		st::debugPrintln(F("SmartThingsWiFi101: Intialized"));
		st::debugPrintln(F(""));
