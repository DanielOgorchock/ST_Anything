//******************************************************************************************
//  File: EX_RGBW_NeoPixelBus_T.h
//  Authors: Allan (vseven) based on EX_Switch_Dim by Dan G Ogorchock
//
//  Summary:  EX_RGBW_NeoPixelBus_T is a templated class which implements the SmartThings "Color Control", "Switch", and "Switch Level" device capabilities.
//			  It inherits from the st::Executor class.
//
//			  Create an instance of this class in your sketch's global variable section
//			  For Example:  st::EX_RGBW_NeoPixelBus_T<FEATURE, METHOD> executor1("rgbSwitch1", PIXEL_COUNT, OUTPUT_PIN);
//			  
...
//
//			  st::EX_RGBW_NeoPixelBus_T<NeoGrbwFeature, NeoEsp32Rmt1800KbpsMethod> executor1("rgbSwitch1", 60, 25);
//			  st::EX_RGBW_NeoPixelBus_T<NeoRgbwFeature, NeoEsp32Rmt2800KbpsMethod> executor2("rgbSwitch2", 150, 26);
//			  st::EX_RGBW_NeoPixelBus_T<NeoGrbwFeature, NeoEsp32Rmt3800KbpsMethod> executor3("rgbSwitch3", 30, 27);
//
//			  st::EX_RGBW_NeoPixelBus_T() constructor requires the following arguments
...
//    2020-08-01  Allan (vseven) Modified the EX_RGBW_Dim for use with addressable LED strips using the NeoPixelBus library
//    2020-08-14  Allan (vseven) Converted the library into a template library to allow multiple different LED strips
//
//******************************************************************************************
#ifndef ST_EX_RGBW_NeoPixelBus_T
#define ST_EX_RGBW_NeoPixelBus_T
...
namespace st
{
	...
	class EX_RGBW_NeoPixelBus_T: public Executor
	{
		private:
			...
			void writeCommandToOutput()
			{
				...
				if (m_bCurrentState == HIGH){
					...
					long number = (long) strtol( &hexstring[1], NULL, 16);
		      			// Split them up into r, g, b, and w values
		      			R = number >> 24;
		      			G = number >> 16 & 0xFF;
		      			B = number >> 8 & 0xFF;
					W = number & 0xFF;
		...
		public:
			...
			EX_RGBW_NeoPixelBus_T(const __FlashStringHelper *name, uint16_t pixelCount, uint8_t outputPin1):
				Executor(name),
			...
			EX_RGBW_NeoPixelBus_T(const __FlashStringHelper *name, uint16_t pixelCount, uint8_t outputPin1, uint8_t outputPin2):
				Executor(name),
			...
			void init()
			{
				...
				if (st::Executor::debug) {
					Serial.println("EX_RGBW_NeoPixelBus_T - " + getName() + " init called. Sending Begin to RGB strip to clear it out.");
				}
			...
			void beSmart(const String &str)
			{
				...
				if (st::Executor::debug) {
					Serial.print(F("EX_RGBW_NeoPixelBus_T::beSmart s = "));
					Serial.println(s);
				}
				if(s==F("on"))
				{
